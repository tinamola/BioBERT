...
def main(args, shuffle_data=True, model=None):
    ...
    #     # not reveal the true answer while get nearest_example
    #     # query=sample['masked_sentences'][0].replace("[MASK]","")
    #     # reveal the true answer while get nearest_example
    #     query = sample['masked_sentences'][0].replace("[MASK]", sample['obj_label'])
    ...
    all_bm25_samples = []
    for line in open('produced_context_bm25/%s.json' % args.relation, 'r'):
        all_bm25_samples.append(json.loads(line))
    if len(all_bm25_samples) > len(all_samples):
        tmp_all_samples1 = all_bm25_samples
        all_bm25_samples = []
        for i in range(len(all_samples)):
            while all_samples[i]['masked_sentences'][0] != tmp_all_samples1[i]['masked_sentences'][0]:
                del tmp_all_samples1[i]
            all_bm25_samples.append(tmp_all_samples1[i])

    samples_batches, sentences_batches, ret_msg = batchify(all_bm25_samples, args.batch_size)  #[[32],[32]] first is whole sample, second is masksed sentence
    #logger.info("\n" + ret_msg + "\n")
