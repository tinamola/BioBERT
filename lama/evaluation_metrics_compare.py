...
def printAttention(vocab,_attentions_bert,_attentions_biobert,token_ids_list,dir):
    token_list, add_list, final_list = [], [], [0]
    for idxx, index in enumerate(token_ids_list):
    ...
    if idxx==len(token_ids_list)-1:  #没有[pad]的sentence
        idxx+=1
    for ex in add_list:
    ...
    plot_attention_heatmap(attentions_mat.sum(axis=1) / attentions_mat.shape[1], token_list, masked_indices_list,
                           t_positions=list(range(len(token_list))),bio="",dir=dir)
...
def get_ranking(log_probs, masked_indices, token_ids, vocab, log_probs1, sample, bert_model=None, biobert_model=None,sentence=None,atten_bert=None, atten_bio=None, label_index = None, index_list = None, topk = 1000, P_AT = 10, print_generation=True):
    ...
    if label_index is not None:
        ...
        if len(ranking_position) >0 and ranking_position[0].shape[0] != 0 and len(ranking_position1) >0 and ranking_position1[0].shape[0] != 0:
            ...
            if rank == 1 and rank1 == 1:
                ...
                _,grad2= biobert_model.calculate_saliency([sentence], [sample])
                lock.release()
                ...
                # print("start----------------------------")
                # print(sample)
                # print("BioBERT successfully predict P@1 but BERT didn't even predict P@10")
                # lock.acquire()
                ...
                printSaliencyFunc(grad1[0], text1, "img/SelfAttenBert@10+_bio@1/", "")
                printSaliencyFunc(grad1[1], text1, "img/AttenBert@10+_bio@1/", "")
                printSaliencyFunc(grad1[2], text1, "img/OutputBert@10+_bio@1/", "")
                printSaliencyFunc(grad2[0], text1, "img/SelfAttenBert@10+_bio@1/", "_Bio")
                printSaliencyFunc(grad2[1], text1, "img/AttenBert@10+_bio@1/", "_Bio")
                printSaliencyFunc(grad2[2], text1, "img/OutputBert@10+_bio@1/", "_Bio")
                situation = 3
...
def printSaliencyFunc(grad, text1, dir, bio):
    grad = torch.abs(grad)
    fig = plt.figure(dpi = 150)
    xticklb = list(text1)
    yticklb = [str(i) if i % 2 == 0 else '' for i in np.arange(0, grad.shape[0], 1)]
    # cmap = sns.diverging_palette(240, 240, as_cmap=True)
    ax = sns.heatmap(grad, xticklabels=xticklb, yticklabels=yticklb, cmap="Blues", annot=True, annot_kws={"size": 6})
    temp = text1[:10] if len(text1) >= 10 else text1
    temp = " ".join(temp)
    temp = temp.replace('"', '\'')
    temp = temp.replace('?', 'Ques')
    temp = temp.replace(':', 'Col')
    fig.savefig('{}.png'.format(dir + temp + bio))  # to avoid '  someone said: "i'm done"  ' give error
    plt.close(fig)
