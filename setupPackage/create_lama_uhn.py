...
class StringMatchFilter(LAMAUHNFilter):
    ...
    def match(self, query):
        if "sub_label" not in query or "obj_label" not in query:  ##########a thing in conceptnet has no sub_label
            return True
        sub_label, obj_label = query["sub_label"], query["obj_label"]
        if self.do_lower_case:  #if set to true, both to lower match, filter it, if set to false, one is upper, one is lower, won't filter it.
            sub_label = sub_label.lower()
            obj_label = obj_label.lower()
        # if obj_label in sub_label:
        #     print(obj_label, ":::",sub_label)
