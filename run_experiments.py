...
#
########################
#########gpt3 context biobert and bert########
########################
# from trainDataGPT3 import main as run_evaluation
# from trainDataGPT3 import load_file
# from gpt3 import main as run_evaluation
# from gpt3 import load_file
########################
#########run these two line need to comment the precision code below########
########################
# from batch_eval_trainingData import main as run_evaluation
# from batch_eval_trainingData import load_file
########################
#########bm25 prompting########
########################
# from batch_eval_KB_completion_elastic import main as run_evaluation
# from batch_eval_KB_completion_elastic import load_file
########################
#########normal########
########################
...
########lama invariant单独的#####
########################
# from batch_eval_KB_completion_INV import main as run_evaluation
# from batch_eval_KB_completion_INV import load_file
...
LMs = [
    ...
    #     "label":"bio_bert",
    #     "models_names":["bert"],
    #     "bert_model_name":"bio-bert-cased",
    #     "bert_model_dir":"pre-trained_language_models/bert/biobert-base-cased-v1.2",
    #     # "bert_model_dir":"pre-trained_language_models/bert/bert_new_3",
    #  },
{
        "lm": "bert",
        "label": "scibert",
        "models_names": ["bert"],
        "bert_model_name": "scibert-cased",
        "bert_model_dir": "pre-trained_language_models/bert/scibert_cased",
    },
]
...
def run_experiments(
    ...
    for relation in relations:
        ...
        if model is None:
            ...
            tm1 = time.time()
            print("\nBuild model ",input_param["label"],"takes time elapsed: ",tm1-tm,"\n")
            vocab_subset = load_vocab(args.common_vocab_filename)
            model.optimize_top_layer(vocab_subset)  #为了elmo
            #################
    ...
    #################
    tm3 = time.time()
    print("\nTime elapsed for probing dataset: ", data_path_pre," using model: ", input_param["label"], " is ", tm3 - tm2)
    print("\n")
...
def get_GoogleRE_parameters():
    ...
    # data_path_pre = "data/Google_RE_UHN/"
...
def get_ConceptNet_parameters(data_path_pre="data/"):
    ################
    ##############split conceptNet, now we only need 16 gb cpu memory
    relations = [{"relation": "test1"},{"relation": "test2"},{"relation": "test3"},{"relation": "test4"},{"relation": "test5"},{"relation": "test6"},
                 {"relation": "test7"},{"relation": "test8"},{"relation": "test9"},{"relation": "test10"},{"relation": "test11"}]
    data_path_pre += "ConceptNet/"
    # data_path_pre += "ConceptNet_UHN/"
    data_path_post = ".jsonl"
    return relations, data_path_pre, data_path_post
...
def get_Squad_parameters(data_path_pre="data/"):
    ...
    data_path_pre += "Squad/"
    # data_path_pre += "Squad_UHN/"
    data_path_post = ".jsonl"
    return relations, data_path_pre, data_path_post

def get_Invariant_parameters(data_path_pre="data/"):
    relations = [{"relation": "invariantLAMA"},{"relation": "invariantLAMA1"},{"relation": "invariantLAMA2"},{"relation": "invariantLAMA3"},{"relation": "invariantLAMA4"},{"relation": "invariantLAMA5"}]
    data_path_pre += "ckl_data/"
    data_path_post = ".csv"
    return relations, data_path_pre, data_path_post
...
def run_all_LMs(parameters):
    for ip in LMs:
        ...
        tm1 = time.time()
        print("\nTime elapsed for ", ip["label"],"is: ",tm1 - tm, "\n")
...
if __name__ == "__main__":
    ...
    # print("5. invariant")
    # parameters = get_Invariant_parameters()
    # run_all_LMs(parameters)
