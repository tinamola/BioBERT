...
def load_file(filename):
    ...
    for row in csvreader:
        sample = {}
        sample["obj_label"] = row[2]
        sample["masked_sentences"] = [row[1].replace("<extra_id_0>", base.MASK)]
...
def main(args, shuffle_data=True, model=None):
    ...
    #args=PARAMETERS = {
        ...
        #         data_path_pre, relation["relation"], data_path_post     eg. "Google_re/place_of_birth.jsonl","test.jsonl"
        #     ),
        #     "common_vocab_filename": "pre-trained_language_models/common_vocab_cased.txt",
        #     "template": "",   #eg."[X] (born [Y]).",
        ...
        #     "threads": -1,
        #     "interactive": False,
        #     "use_negated_probes": use_negated_probes,  False
        # "label": "bert_large",
        # "models_names": ["bert"],
    ...
    # )
    def generate_context_func(sample):
        thisdict = {'was born in': 'P19',
                    'died in': 'P20',
                    'is a subclass of': 'P279',
                    'The official language of': 'P37',
                    'was originally aired on': 'P449',
                    'shares border with': 'P47',
                    'is named after': 'P138',
                    'The original language of': 'P364',
                    'consists of': 'P527',
                    'is produced by': 'P176',
                    'citizen': 'P27',
                    'was written in': 'P407',
                    'is located in': ['P30','P131','P17','P276'],
                    'is developed by': 'P178',
                    'is the capital of': 'P1376',
                    'used to communicate in': 'P1412',
                    'used to work in': 'P937',
                    'is affiliated with the': 'P140',
                    'The native language of': 'P103',
                    'are twin cities': 'P190',
                    'is a legal term in': 'P1001',
                    'was created in': 'P495',
                    'The capital of': 'P36',
                    'was founded in': 'P740',
                    'is part of': 'P361'}
        for key in thisdict:
            if key in sample["masked_sentences"][0]:
                relations=thisdict[key]
                if type(relations) != list:
                    for line in open('produced_context_gpt3/%s.json' % relations, 'r'):
                        if json.loads(line)['masked_sentences'][0] == sample["masked_sentences"][0]:
                            sample['masked_sentences'].append(json.loads(line)['masked_sentences'][1])
                            return sample
                else:
                    for relation in relations:
                        for line in open('produced_context_gpt3/%s.json' % relation, 'r'):
                            if json.loads(line)['masked_sentences'][0] == sample["masked_sentences"][0]:
                                sample['masked_sentences'].append(json.loads(line)['masked_sentences'][1])
                                return sample
        print(sample)
    def mask_query_func(example):
        ...
        return example
    # all_samples = list(map(generate_context_func, all_samples))
    all_samples1 = list(filter(lambda s: s is not None, all_samples))
    # all_samples1 = list(map(mask_query_func, all_samples1))
    # with open('trainDataInvariantBM25/%s.txt' % args.relation, "w", encoding='utf-8') as fp:
    #     for example in all_samples1:
    ...
    for i in tqdm(range(len(samples_batches))):
        ...
        (
            ...
            token_ids_list,
            masked_indices_list,
        ) = model.get_batch_generation(sentences_b, logger=logger,output_atten=False)
